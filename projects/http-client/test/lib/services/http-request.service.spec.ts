...
import { mockHttpParameters } from '../mocks/http-parameters.mock';
import { HttpRequestClientServices } from './../../../src/lib/services/http-request.service';
...
describe('HttpRequestClientServices', () => {
  ...
  test('should execute get with headerParams', () => {
    service.execute<mockReturnHttp[]>(HttpMethod.Get, '/', mockHttpParameters).subscribe({
  ...
  test('should execute get get with headerParams and queryString', () => {
    service.execute<mockReturnHttp[]>(HttpMethod.Get, '/', mockHttpParameters, mockHttpParameters).subscribe({
      next: (x: mockReturnHttp[]) => {
        expect(x.length).toBe(2);
        expect(x).toEqual(dummyRequest);
      },
  ...
  test('should execute delete', () => {
    ...
    service.execute<string>(HttpMethod.Delete, '/', undefined, mockHttpParameters).subscribe({
      next: (x: string) => {
